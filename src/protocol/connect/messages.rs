...
impl Serialize for Message {
    ...
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let json_message = WireMessage::from(self.clone());
        let json =
...
impl From<Message> for WireMessage {
    /// Converts to a `WireMessage` from a [`Message`](struct.Message.html).
...
impl TryFrom<WireMessage> for Message {
    ...
    fn try_from(message: WireMessage) -> Result<Self, Self::Error> {
        let variant = match message {
            WireMessage::WithContents(stanza, channel, contents) => match stanza {
